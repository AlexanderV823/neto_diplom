...
Процедура ВызватьАвтоначисление(Ссылка, Параметры) Экспорт
    ...
    Если Параметры.Форма.Объект.Выплаты.Количество() > 0 Тогда //Необходимо упростить получение ТЧ, либо убрать проверку в целом.
        ...
        ОбработчикОповещения = Новый ОписаниеОповещения("ВызватьАвтоначислениеПродолжить", Параметры.Форма);
    ...
    Иначе
        ...
        ВызватьАвтоначислениеНаСервере();
...
&НаСервере
Процедура ВызватьАвтоначислениеНаСервере()
    
        ДокументОбъект = РеквизитФормыВЗначение("Объект");
        ДокументОбъект.ВыполнитьАвтоначисление();     
