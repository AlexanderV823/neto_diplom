...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    Если Выплаты.Количество() = 0 Или Проведен = Истина Тогда
	...
	Пока Выборка.Следующий() Цикл
        ...
        Если Выборка.СуммаОстаток = 0 Тогда
            ...
            ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ТекстСообщения, Выборка.СотрудникПредставление), , СтрШаблон("Выплаты[%1].Сотрудник", Выборка.НомерСтроки), "Объект", Отказ);
        ...
        ИначеЕсли Выборка.Сумма > Выборка.СуммаОстаток Тогда
            ...
            ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ТекстСообщения, Выборка.СотрудникПредставление, Выборка.СуммаОстаток), , СтрШаблон("Выплаты[%1].Сотрудник", Выборка.НомерСтроки), "Объект", Отказ);
