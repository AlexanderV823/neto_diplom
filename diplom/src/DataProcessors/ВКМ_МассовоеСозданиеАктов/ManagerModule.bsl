...
Функция СоздатьДокументыРеализаций(Период) Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Иначе                
            ...
            //"Тут подразумевается, что если уже есть документ - он должен дозаполниться."            
            ДокументОбъект = Выборка.Реализация.ПолучитьОбъект();
            ДокументОбъект.ВКМ_ВыполнитьАвтозаполнение(Выборка.Реализация);
...
Функция СозданиеРеализации(Выборка, Дата)
    ...
    НовыйДокумент.Заполнить(Неопределено); //"при создании Реализации необходимо вызывать стандартный алгоритм заполнения"
                                           //Поскольку у нас нет понимания о документе-основании (ЗаказПокупателя), то передаем Неопределено.
    //Остальные реквизиты заполняем тем, что есть:
    ...
    Иначе
        ...
        ТекстСообщения = НСтр("ru = 'Не удалось провести документ реализации к договору: %1.
                                    |Не все обязательные поля заполнены!'");
        
        ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ТекстСообщения, Выборка.Договор)); 
        //Ошибка будет выводиться пока поле Основание будет обязательным к заполнению 
        //или не будет понятно какое основание выбирать в запросе.
            
    КонецЕсли;

    //НовыйДокумент.ВКМ_ВыполнитьАвтозаполнение(НовыйДокумент.Ссылка); //Логично было бы новый документ тоже сразу заполнить.
